...
function formatSerial(base32: string): string {
  const groups: string[] = [];
...
export function generateLicense(clientId: string, clientName: string, months: number, maxActivations: number, secretKey: string) {
  const now = new Date();
  ...
  const hmac = hmacSha256(secretKey, payload).substring(0, 16);
...
export function validateLicense(serial: string, secretKey: string) {
  try {
    ...
    if (hmacSha256(secretKey, payload).substring(0, 16) !== expectedHmac) return { valid: false, error: 'Invalid serial signature' };
  ...
  } catch { return { valid: false, error: 'Invalid serial format' }; }
}

export function getExpirationInfo(serial: string, secretKey: string) {
